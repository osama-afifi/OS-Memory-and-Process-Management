...
void check_boot_pgdir()
{
	...
	{
		//cprintf("i = %x, arg 1  = %x, arg 2 = %x \n",i, check_va2pa(ptr_page_directory, READ_ONLY_FRAMES_INFO + i), K_PHYSICAL_ADDRESS(frames_info) + i);
...
uint32 check_va2pa(uint32 *ptr_page_directory, uint32 va)
{
	...
	if (!(*dirEntry & PERM_PRESENT))
		return ~0;
	p = (uint32*) K_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(*dirEntry));
...
//	ptr_page_directory[1] = ptr_page_directory[PDX(KERNEL_BASE+PAGE_SIZE*1024)];
