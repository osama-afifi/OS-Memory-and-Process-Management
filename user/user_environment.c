...
DECLARE_START_OF(fos_fib)
uint32 kernel_cr3=0;
//User Programs Table
//The input for any PTR_START_OF macro must be the ".c" filename of the user program
struct UserProgramInfo userPrograms[] = {
{ "fos_helloWorld", "Created by FOS team, fos@nowhere.com", PTR_START_OF(fos_helloWorld), 0,10 },
{ "fos_add", "Created by FOS team, fos@nowhere.com", PTR_START_OF(fos_add), 0,1},
{ "fos_alloc", "Created by FOS team, fos@nowhere.com", PTR_START_OF(fos_alloc), 0,1},
{ "fos_input", "Created by FOS team, fos@nowhere.com", PTR_START_OF(fos_input), 0,2000},
{ "fos_game", "Created by FOS team, fos@nowhere.com", PTR_START_OF(game), 0,1},
{ "fos_fib", "Created by FOS team, fos@nowhere.com", PTR_START_OF(fos_fib), 0,50},
};
		...
		for(i = seg->size_in_file ; i < seg->size_in_memory ; i++)
{
	...
	//TODO: [Assignment5.Hands-on] allocate different stack pages/program
	//write your code here after commenting the panic line
...
for(i=0;i<ptr_user_program_info->stack_size;++i)
{
		...
		void* ptr_user_stack_bottom = (void *) (USTACKTOP - (i+1)*PAGE_SIZE);
		int ret = map_frame(e->env_pgdir, pp, ptr_user_stack_bottom, PERM_USER
				| PERM_WRITEABLE);
...
void free_inKernel(uint32* p)
{
	if (p != NULL) {
		struct Frame_Info *ptr = to_frame_info(K_PHYSICAL_ADDRESS(p));
		ptr->references = 0;
		free_frame(ptr);
		//No need for this line ptr_dir[PDX(va)] = 0;
		//because the directory itself will be deleted
	}
	return ;
}
//
		...
		if (ptr_Dir[j] & PERM_PRESENT) {
			...
			for (k = 0; k < 1024; ++k)
				if (pTable[k] & PERM_PRESENT)
					unmap_frame(ptr_Dir, (uint32*) ((j << 22) + (k << 12)));
			free_inKernel(pTable);
		}
	// [3] free the page directory of the environment
	free_inKernel(ptr_Dir);
	// [4] switch back to the kernel page directory
	lcr3((uint32)ptr_page_directory-KERNEL_BASE);
